...
for (my $i=0; $i<scalar(@$data); $i++) {
	print FILE "IN THE LOOP Count [$i] \n\n";
...
# don't calculate fines beyond cutover date as outstanding amts were converted for bcl

#       my $cutover_date;
#       if ($branchcode eq 'HD'){
#               $cutover_date=C4::Dates->new('2011-03-22','iso');
#       } elsif ($branchcode eq 'BL') {
#               $cutover_date=C4::Dates->new('2011-03-25','iso');
#       } elsif ($branchcode eq 'CH'){
#               $cutover_date=C4::Dates->new('2011-04-04','iso');
#       } elsif ($branchcode eq 'CB') {
#               $cutover_date=C4::Dates->new('2011-04-05','iso');
#       } elsif ($branchcode eq 'KD') {
#                $cutover_date=C4::Dates->new('2011-04-05','iso');
#        } elsif ($branchcode eq 'PN') {
#                $cutover_date=C4::Dates->new('2011-04-14','iso');
#        }



#       my $cutover_days=Date_to_Days(split(/-/,$cutover_date->output('iso')));
#       if ($cutover_days ge $datedue_days){
#               $datedue=$cutover_date;
#       }
    ...
    my ($amount,$type,$daycounttotal,$daycount)=
         ...
         if($amount <= 0) {
         	print FILE "Date Due:: $datedue\n";
         	print FILE "TODAY DATE:: $today\n\n";
         	print FILE "Cat code of borrower :: $borrower->{'categorycode'}\n\n"
         	print FILE "Branch code of borrower:: $branchcode\n\n"
         	
         }       
        
        print FILE "Returns by CalcFine:",$amount,$type,$daycounttotal,$daycount,"\n\n";
